...
export const addScoreUpdate = async (countyName, competitionId, fixtureId, scoreData) => {
  try {
    // pat: counties/[county]/competitions/[competitionId]/fixtures/[fixtureId]/scores/[newScoreId]
    ...
    // add a small time delay to prevent duplicates
    const now = new Date()
    const timestamp = Timestamp.fromDate(new Date(now.getTime() + Math.random() * 1000))
